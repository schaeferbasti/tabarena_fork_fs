...
class DISR:
    ...
    def fit_transform(self, X: pd.DataFrame, y: pd.Series, model, n_max_features, **kwargs) -> pd.DataFrame:
        ...
        feature_ranking = self.disr(X_np, y_np, n_selected_features=n_max_features)
    ...
    def disr(self, X, y, **kwargs):
        ...
        while True:
            ...
            for i in range(n_features):
                if i not in F:
                    f = X[:, i]
                    t2 = self.midd(f_select, y) + self.cmidd(f, y, f_select)
                    t3 = self.entropyd(f) + self.conditional_entropy(f_select, f) + (
                                self.conditional_entropy(y, f_select) - self.cmidd(y, f, f_select))
