...
class LaplacianScore:
    """LaplacianScore feature selector (only flip)"""
    ...
    def fit_transform(self, X: pd.DataFrame, y: pd.Series, model, n_max_features, **kwargs) -> pd.DataFrame:
        ...
        X_selected = self.laplacian_score(X, y, model, n_max_features, init_feature_choice, **kwargs)
    ...
    def laplacian_score(self, X_train, y_train, model, n_max_features, feature_indices, **kwargs):
        return X_train
