...
class mRMR(AbstractFeatureSelector):
    ...
    def _fit_transform(self, X: DataFrame, y: Series, model, n_max_features: int, **kwargs) -> tuple[DataFrame, dict]:
        ...
        from tabarena.benchmark.feature_selection_methods.ag.mrmr.method.mRMRFS import mRMRFS
        self._mrmr = mRMRFS()
        # Time limit
        if "time_limit" in kwargs and kwargs["time_limit"] is not None:
            ...
            if kwargs["time_limit"] <= 0:
                ...
                if n_max_features is not None and len(X.columns) > n_max_features:
                    ...
                    return X
        X_out = self._mrmr.fit_transform(X, y, model, n_max_features, **kwargs)
    ...
    def _transform(self, X: DataFrame, *, is_train: bool = False) -> DataFrame:
        if is_train:
            X = self._mrmr.fit_transform(X, self._y, self._model, self._n_max_features)
            self._selected_features = list(X.columns)
        else:
            X = X[self._mrmr._selected_features]
        return X
