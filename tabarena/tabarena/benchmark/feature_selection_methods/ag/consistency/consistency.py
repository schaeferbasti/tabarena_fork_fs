...
class ConsistencyFeatureSelector(AbstractFeatureSelector):
    ...
    def _fit_feature_selection(
        ...
        # Consistency selects a subset without a natural per-feature value -> NaN
        # (selected by the method, unlike random-fallback fills which are None).
        return [{"feature": str(f), "value": float("nan")} for f in selected_features]
