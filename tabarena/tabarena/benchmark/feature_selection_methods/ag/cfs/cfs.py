...
class CFSFeatureSelector(AbstractITFeatureSelector):
    ...
    def _fit_feature_selection(
        ...
        *, 
        ...
        prev_merit = 0.0
        values = []  # merit gain contributed by each selected feature (its "value")
        ...
        while len(F) < self.max_features:
            ...
            if idx == -1:
                break
            F.append(idx)
            values.append(merit - prev_merit)  # this feature's contribution to the subset merit
            prev_merit = merit
        return [
            {"feature": str(self._original_features[i]), "value": float(v)}
            for i, v in zip(F, values)
        ]
